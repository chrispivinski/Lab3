...
public class Hand {
	...
	public String toString() {
		String msg = "";
		msg = msg + this.getCardsInHand().get(0).toString() + "\n";
		msg = msg + this.getCardsInHand().get(1).toString() + "\n";
		msg = msg + this.getCardsInHand().get(2).toString() + "\n";
		msg = msg + this.getCardsInHand().get(3).toString() + "\n";
		msg = msg + this.getCardsInHand().get(4).toString();
		return msg;
	}
	...
	public static ArrayList<Hand> ExplodeHands(Hand h) {
		...
		for (int i = 0; i < Jokers; i++) {
			ReturnHands = SubHands(ReturnHands, i);
		}

		return ReturnHands;
	}

	private void setCardsInHand(ArrayList<Card> c) {
		this.CardsInHand = c;
	}

	private static ArrayList<Hand> SubHands(ArrayList<Hand> hands, int location) {
		ArrayList<Hand> temp = new ArrayList<Hand>();
		for (Hand h : hands) {
			if (h.getCardsInHand().get(location).geteRank() == eRank.JOKER) {
				for (eSuit suit : eSuit.values()) {
					for (eRank rank : eRank.values()) {
						if ((suit != eSuit.JOKER) && (rank != eRank.JOKER)) {
							Hand tmpHand = new Hand();
							tmpHand.setCardsInHand(h.getCardsInHand());
							tmpHand.getCardsInHand().set(location, new Card(suit, rank, 1));
							temp.add(tmpHand);
						}
					}
				}
